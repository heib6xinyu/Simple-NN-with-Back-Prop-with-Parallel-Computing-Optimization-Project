...
file_path = './datasets/agaricus-lepiota.txt'  # Replace with your file path
...
with open(file_path, 'r') as file:
    for line in file:
        ...
        y = int(parts[0])
        X = [int(x) for x in parts[1].split(',')]
        data.append([y] + X)

# Convert to DataFrame
df = pd.DataFrame(data, columns=['target'] + [f'feature_{i}' for i in range(len(data[0]) - 1)])
